...
namespace Compression.AC {
    ...
    public class ArithmeticCoder {
        ...
        private readonly BitString _bitString = new BitString();

        private int _followBits;
        private readonly Interval _interval = new Interval(0, MAX_INTERVAL, MAX_INTERVAL);
        ...
        public void FinalizeInterval() {
            ...
            var toEncode = _interval.Upper > _interval.Max - _interval.Lower ? UnevenByte.One : UnevenByte.Zero;
            _bitString.Append(toEncode);

            // Encode the complement follow bits times
            for (; _followBits > 0; --_followBits) _bitString.Append(!toEncode);
